...
const HEADER = '<html><title>IndiAves</title><body><script async="" src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>'
const DIV_START = '<div class=flex-box style="display:flex; flex-direction: row;">'
const DIV_END = '</div>'
...
async function enrichData() {
    /*These have been commented out since the scheduler in Python doesn't seem to work in GCP*/
    //TODO enable this data fetch and enable scheduler in python
...
export async function createHTML(v2 = false, search_term, dayFrom, dayTo, items = 10) {
    ...
    let filteredData = [];
    if (v2) {
        filteredData = await got.get(`http://127.0.0.1:5000/search/${search_term}/count/${items}/fromDate/${dayFrom}/toDate/${dayTo}`).json()
    }
    else {
    ...
    for (const item of filteredData.slice(0, items)) {
        requests.push(got.get(`https://publish.twitter.com/oembed?url=${base_url}/${item.id}&maxwidth=500&omit_script=1`).json())
    }
